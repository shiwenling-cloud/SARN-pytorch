...
class CALayer_1(nn.Module):
	def __init__(self, channel, reduction=16, reduction_1=8):
		...
		self.conv_c = nn.Conv2d(channel, channel, 1, padding=0, bias=True)

	def forward(self, x):
		h_size = x.size(2)
		w_size = x.size(3)
		self.conv_du_h1 = nn.Sequential(
			nn.Conv2d(h_size, h_size // 8, 1, padding=0, bias=True),
			nn.ReLU(inplace=True),
			nn.Conv2d(h_size // 8, h_size, 1, padding=0, bias=True),
			nn.Sigmoid()
		).cuda()

		self.conv_du_w1 = nn.Sequential(
			nn.Conv2d(w_size, w_size // 8, 1, padding=0, bias=True),
			nn.ReLU(inplace=True),
			nn.Conv2d(w_size // 8, w_size, 1, padding=0, bias=True),
			nn.Sigmoid()
		).cuda()
